...
def do_start_chat(attendees_ids, text):
    if not attendees_ids:
        return {"error": "attendees_ids is required (comma-separated user IDs)"}
    if not text or not text.strip():
        return {"error": "text is required"}
...
def run(action, **kwargs):
    if not UNIPILE_DSN or not UNIPILE_API_KEY or not UNIPILE_ACCOUNT_ID:
        return {"error": "LinkedIn not connected. Connect your LinkedIn account via the Channels/Gateways section."}
...
# --- Oya sandbox entrypoint: reads from INPUT_JSON env var ---
